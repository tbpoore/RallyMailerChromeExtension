...
function mapElementsToArtifactObjects(artifactElements, artifactLinkSelector, artifactNameSelector) {
...
function getSelectedSearchArtifacts() {
    ...
    return $.makeArray(mapElementsToArtifactObjects(selectedArtifacts, artifactLinkSelector, artifactNameSelector)).reverse();
}
...
function getInjectableArtifactElements(artifactElementSelector) {
    ...
    var injectableElements = allInjectableElements.filter(function (index, element) {
        return $(element).find('.rally-mailer-select').first().length === 0;
    });
...
var rallyArtifactHelper = {
    ...
    getAllSelectedArtifacts: function () {
        ...
        return [].concat.apply([], [treeGridArtifacts, searchArtifacts, browserArtifacts]);
    },
